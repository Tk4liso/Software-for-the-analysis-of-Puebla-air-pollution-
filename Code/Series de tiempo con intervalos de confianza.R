...
# ==== Facetado dividido en 2 ====
group1 <- c("CO", "NO2", "O3")
group2 <- c("SO2", "PM10", "PM25")

# Filtrar datos por grupo
df_group1 <- df_facet_pred %>% filter(Pollutant %in% group1)
df_group2 <- df_facet_pred %>% filter(Pollutant %in% group2)

#Plot para grupo 1
ggplot(df_group1, aes(x = Date, y = Concentration, color = Type)) + 
  geom_line() +
  geom_ribbon(data = df_group1 %>% filter(Type == "Forecast"),
              aes(x = Date, ymin = Lower, ymax = Upper, fill = Type),
              alpha = 0.2, inherit.aes = FALSE) +
  facet_wrap(~ Pollutant, scales = "free_y") +
  labs(
    title = "Pollutant Forecasts – Group 1: CO, NO2, O3",
...
#Plot para grupo 2
ggplot(df_group2, aes(x = Date, y = Concentration, color = Type)) + 
  geom_line() +
  geom_ribbon(data = df_group2 %>% filter(Type == "Forecast"),
              aes(x = Date, ymin = Lower, ymax = Upper, fill = Type),
              alpha = 0.2, inherit.aes = FALSE) +
  facet_wrap(~ Pollutant, scales = "free_y") +
  labs(
    title = "Pollutant Forecasts – Group 2: SO2, PM10, PM25",
    x = "Date", y = "Concentration"
  ) +
  scale_color_manual(values = c("Observed" = "gray", "Forecast" = "red")) +
  scale_fill_manual(values = c("Forecast" = "red")) +
