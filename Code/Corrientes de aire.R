...
ggplot(data_plot, aes(x = fecha, y = direccion_grados)) +
  ...
  labs(
    title = "Wind direction throughout the year",
    x = "Month",
    y = "Direction (°)") +
  theme_minimal() +
...
# Visualization 1: Wind direction histogram
ggplot(data, aes(x = direccion_grados)) +
  geom_histogram(binwidth = 10, fill = "skyblue", color = "black") +
  geom_vline(xintercept = c(210, 250), color = "red", linetype = "dashed") +
  labs(
    title = "Wind Direction Distribution (2023)",
    x = "Wind Direction (°)",
    y = "Frequency"
  ) +
  theme_minimal()

# Visualization 2: Wind direction over time
library(scales)  # For date labels
data_plot <- data
data_plot$fecha <- as.Date(data_plot$fecha, format = "%d/%m/%Y")
...
# --- Comparison: Burning season vs rest of the year ---
...
data_plot <- data_plot %>%
  mutate(
    temporada_quema = ifelse(mes %in% c(12, 1, 2, 3, 4), "Burning", "Non-burning"),
    favorable = direccion_grados >= 210 & direccion_grados <= 250
  )

# Comparative table
tabla_comparativa <- data_plot %>%
  mutate(season_label = ifelse(
    temporada_quema == "Burning",
    "Sugarcane Burning Season (Dec–Apr)",
    "Rest of the Year (May–Nov)"
  )) %>%
  group_by(season_label) %>%
  summarise(
    total_days = n(),
    favorable_days = sum(favorable, na.rm = TRUE),
    favorable_percentage = round(favorable_days / total_days * 100, 1)
  )

print(tabla_comparativa)

# Comparative bar plot
ggplot(tabla_comparativa, aes(x = season_label, y = favorable_percentage, fill = season_label)) +
  geom_col(width = 0.6, color = "black") +
  geom_text(aes(label = paste0(favorable_percentage, "%")), vjust = -0.5, size = 5) +
  scale_fill_manual(values = c(
    "Sugarcane Burning Season (Dec–Apr)" = "#f4a261",
    "Rest of the Year (May–Nov)" = "#a8dadc"
  )) +
  labs(
    title = "Sugarcane Burning Season vs Rest of the Year",
    x = "",
    y = "Days with Wind from Izúcar toward Puebla (%)"
  ) +
  theme_minimal() +
  theme(legend.position = "none", text = element_text(size = 13))
